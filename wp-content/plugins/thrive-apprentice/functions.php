...
function tva_tcb_version_warning() {
	return include __DIR__ . '/views/tcb_version_incompatible.php';
}
...
function tva_get_localization() {
	...
	return array(
		...
		'is_sendowl_connected' => TVA_SendOwl::is_connected(),
	);
}
...
/**
 * Append customers list to data to be localized
 *
 * @param array $data to be localized
 *
 * @return array
 */
function tva_admin_localize_customers( $data ) {

	foreach ( TVA_Customer::get_list() as $user ) {
		$data['data']['customers'][] = $user->json_serialize();
	}
